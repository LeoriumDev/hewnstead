...
TEST_CASE("raycast hits cell in front") {
    ChunkManager cm;
    auto chunk = cm.loadChunk({.x = 0, .y = 0, .z = 0});
    chunk->set(5, 0, 0, blocks::Stone);
    auto hit = raycast(cm, {0.5F, 0.5F, 0.5F}, {1.0F, 0.0F, 0.0F}, 10.0F);
...
TEST_CASE("raycast respects max distance") {
    ...
    auto hit = raycast(cm, {0.5F, 0.5F, 0.5F}, {1.0F, 0.0F, 0.0F}, 5.0F);
...
TEST_CASE("raycast in negative direction") {
    ...
    auto hit = raycast(cm, {3.5F, 0.5F, 0.5F}, {-1.0F, 0.0F, 0.0F}, 10.0F);
    REQUIRE(hit.has_value());
...
TEST_CASE("raycast with zero direction component") {
    ...
    auto hit = raycast(cm, {0.5F, 0.5F, 0.5F}, {1.0F, 1.0F, 0.0F}, 20.0F);
    REQUIRE(hit.has_value());
...
TEST_CASE("raycast diagonal direction") {
    ...
    auto hit = raycast(cm, {0.5F, 0.5F, 0.5F}, dir, 10.0F);
    REQUIRE(hit.has_value());
...
TEST_CASE("raycast origin inside solid block") {
    ...
    auto hit = raycast(cm, {5.5F, 5.5F, 5.5F}, {1.0F, 0.0F, 0.0F}, 10.0F);
    REQUIRE(hit.has_value());
